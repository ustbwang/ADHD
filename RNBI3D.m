function  [Allpoints2, Allpointsxvaule] = RNBI3D(C, A, b, lb, ub)
%problem description:
%min  Cx,
%s.t. Ax <=b,
%     ub> x > lb.
...
coverfacever = v(1:3,:);
coverplane = planeEquation(coverfacever, 3);
 % Produce grid points
...
for i = 1: NofSampleP
    ...
    fprintf(fid, '%12.6f\t %12.6f\t %12.6f\t', rayintial(1), rayintial(2),rayintial(3));
    f=[zeros(n,1);1];
    lbnew = [lb; 0];
    ubnew = [ub;Inf];
...
normal = zeros(3,1);
refpt = zeros(3,1); % reference point on the plane.
for i = 1: nverts
    u = vers(i,:);
    temp = mod(i+1,nverts);
    if temp == 0
